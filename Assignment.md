...
`Reality 😅`: Turns out Git Rebase replays commits one by one, so conflicts can occur per commit but it actually gives finer control over resolving them compared to a messy merge.
